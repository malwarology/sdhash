...
//
// Because every stage is deterministic, regenerating the corpus and
// re-accumulating the anchor reproduces the same 32-byte value, and any
// regression in generation, digest computation, or scoring changes it.
...
// bdgDefaultCategories returns the canonical category list. Order is
// load-bearing: it determines the order in which the shared seed RNG
// is consumed, so it must never change.
...
// planNormalCorpus is the corpus the corpushash test hashes. filesPerType is
// the global count; per-category overrides in cats take precedence (only "large"
// overrides in the default set). Only the seed RNG is consumed here — no bytes
// are generated.
...
// planMixedbagCorpus is the corpus the corpuscompare test hashes. Unlike normal
// generation, mixedbag ignores per-category count overrides (every category
// gets perCat) but honors per-category min/max size overrides.
...
// ---------------------------------------------------------------------------
// SHA-256 anchor accumulation
// ---------------------------------------------------------------------------

// structural control bytes: None appears in sdhash output data, so they are safe delimiters.
const (
